...
Elm.Main.make = function (_elm) {
   ...
   var Input = F4(function (a,b,c,d) {    return {dims: a,mousePos: b,isDown: c,delta: d};});
   var input = A2($Signal.sampleOn,delta,A5($Signal.map4,Input,$Window.dimensions,$Mouse.position,$Mouse.isDown,delta));
   var view = function (_p0) {
      var _p1 = _p0;
      var _p3 = _p1.room;
      var _p2 = _p1.ball;
      var wall = $Graphics$Collage.solid($Color.gray);
      var h = $Basics.round(_p3.h + 2 * _p3.th);
      var w = $Basics.round(_p3.w + 2 * _p3.th);
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "cursor",_1: _p1.hand.isGrabbing ? "-webkit-grabbing" : "-webkit-grab"}]))]),
      _U.list([$Html.fromElement(A3($Graphics$Collage.collage,
      w,
      h,
      _U.list([A2($Graphics$Collage.outlined,
              _U.update(wall,{width: _p3.th,cap: $Graphics$Collage.Padded}),
              A2($Graphics$Collage.rect,_p3.w + _p3.th,_p3.h + _p3.th))
              ,A2($Graphics$Collage.move,
              {ctor: "_Tuple2",_0: _p2.q.x,_1: _p2.q.y},
              A2($Graphics$Collage.filled,$Color.red,$Graphics$Collage.circle(_p2.r)))])))]));
   };
   var absmin = F2(function (x,y) {    return A2($Basics.min,$Basics.abs(x),$Basics.abs(y));});
   var sign = function (x) {    return _U.cmp(x,0) > 0 ? 1 : _U.cmp(x,0) < 0 ? -1 : 0;};
   var tension = F3(function (u,_p4,n) {
      var _p5 = _p4;
      var _p7 = _p5.y;
      var _p6 = _p5.x;
      return {x: (0 - sign(_p6)) * A2(absmin,_p6,u * n.y),y: (0 - sign(_p7)) * A2(absmin,_p7,u * n.x)};
   });
   var resist = function (_p8) {    var _p9 = _p8;return $Basics.sqrt(_p9.k) / _p9.qFactor;};
   var normal = F3(function (room,dist,v) {    return _U.cmp(dist,0) < 0 ? 0 : (0 - room.k) * dist - resist(room) * v;});
   var updateRoom = F2(function (_p10,room) {
      var _p11 = _p10;
      return _U.update(room,{w: $Basics.toFloat(_p11._0) - 2 * room.th,h: $Basics.toFloat(_p11._1) - 2 * room.th});
   });
   var iterate = F3(function (n,f,x) {
      iterate: while (true) {
         var _p12 = n;
         if (_p12 === 0) {
               return x;
            } else {
               var _v5 = n - 1,_v6 = f,_v7 = f(x);
               n = _v5;
               f = _v6;
               x = _v7;
               continue iterate;
   ...
   var Model = F3(function (a,b,c) {    return {ball: a,room: b,hand: c};});
   var Hand = F4(function (a,b,c,d) {    return {q: a,isGrabbing: b,k: c,qFactor: d};});
   var Room = F6(function (a,b,c,d,e,f) {    return {h: a,w: b,th: c,k: d,qFactor: e,u: f};});
   var Ball = F5(function (a,b,c,d,e) {    return {m: a,r: b,q: c,v: d,a: e};});
   var Vector = F2(function (a,b) {    return {x: a,y: b};});
   _op["<+>"] = F2(function (l,m) {    return A2(Vector,l.x + m.x,l.y + m.y);});
   _op["<*>"] = F2(function (k,l) {    return A2(Vector,k * l.x,k * l.y);});
   var moveBall = F2(function (dt,ball) {    return _U.update(ball,{q: A2(_op["<+>"],A2(_op["<*>"],dt,ball.v),ball.q)});});
   var accelerate = F2(function (dt,ball) {    return _U.update(ball,{v: A2(_op["<+>"],A2(_op["<*>"],dt,ball.a),ball.v)});});
   var vzero = A2(Vector,0,0);
   var elastic = F3(function (_p13,q$,v) {
      var _p14 = _p13;
      var _p15 = _p14.isGrabbing;
      if (_p15 === false) {
            return vzero;
         } else {
            var r = 0 - resist(_p14);
            var dq = A2(_op["<+>"],A2(_op["<*>"],-1,q$),_p14.q);
            return A2(_op["<+>"],A2(_op["<*>"],_p14.k,dq),A2(_op["<*>"],r,v));
         }
   });
   var g = A2(Vector,0,-1000);
   var setForces = F4(function (room,hand,dt,_p16) {
      var _p17 = _p16;
      var _p20 = _p17.v;
      var _p19 = _p17.r;
      var _p18 = _p17.q;
      var fNormal = {x: A3(normal,room,_p18.x + _p19 - room.w / 2,_p20.x) - A3(normal,room,0 - _p18.x + _p19 - room.w / 2,0 - _p20.x)
                    ,y: A3(normal,room,_p18.y + _p19 - room.h / 2,_p20.y) - A3(normal,room,0 - _p18.y + _p19 - room.h / 2,0 - _p20.y)};
      var fTension = A3(tension,room.u,A2(_op["<*>"],1 / dt,_p20),fNormal);
      var fHand = A3(elastic,hand,_p18,_p20);
      return _U.update(_p17,{a: A2(_op["<+>"],A2(_op["<+>"],A2(_op["<+>"],g,fHand),fNormal),fTension)});
   });
   var update = F2(function (_p22,_p21) {
      var _p23 = _p22;
      var _p27 = _p23.mousePos;
      var _p24 = _p21;
      var _p26 = _p24;
      var dt = 1.0e-3;
      var newRoom = A2(updateRoom,_p23.dims,_p26.room);
      var newHand = _U.update(_p24.hand,
      {q: {x: $Basics.toFloat($Basics.fst(_p27)) - (newRoom.w / 2 + newRoom.th),y: newRoom.h / 2 + newRoom.th - $Basics.toFloat($Basics.snd(_p27))}
      ,isGrabbing: _p23.isDown});
      return _U.update(_p26,
      {ball: A3(iterate,
      $Basics.round(_p23.delta / dt),
      function (_p25) {
         return A2(accelerate,dt,A4(setForces,newRoom,newHand,dt,A2(moveBall,dt,_p25)));
      },
      _p26.ball)
      ,room: newRoom
      ,hand: newHand});
   });
   var initial = {ball: A5(Ball,1,25,vzero,A2(Vector,500,1000),g),room: A6(Room,0,0,20,10000,10,2.0e-2),hand: A4(Hand,vzero,false,500,2)};
   var main = A2($Signal.map,view,A3($Signal.foldp,update,initial,input));
